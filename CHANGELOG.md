...
- Exposed the existing Receipt Reliability Gate comparison from the Receipt Extraction Benchmark debug screen.
- Added a read-only PASS/FAIL report view showing coverage, recovered-orphan, remaining-orphan, and regression-status comparisons generated by the existing gate.
